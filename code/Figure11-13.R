...
return0<-ts(return)
dim(return0)
cormatrix<-cor(return0,method="spearman")
W<-matrix(0,71,71)
for (i in 1:71) {
  W[i,]<-prop.table(cormatrix[i,])
}
return0<-t(return0)
W<-as.matrix(W)
X<-return0
...
###############################
####################################################
qfit1<-CQEst(tau=0.1,p=1,q=1,L=30,X=Y,B=1000,W=W)
phybrid1<-2*pt(-abs(qfit1$thetahat/qfit1$sdE),df=T-4)
write.csv(qfit1$thetahat,file="coefrobust-tau01.csv")
write.csv(phybrid1,file="pvaluerobust-tau01.csv")
######################################################
qfit2<-CQEst(tau=0.25,p=1,q=1,L=30,X=Y,B=1000,W=W)
phybrid2<-2*pt(-abs(qfit2$thetahat/qfit2$sdE),df=T-4)
write.csv(qfit2$thetahat,file="coefrobust-tau025.csv")
write.csv(phybrid2,file="pvaluerobust-tau025.csv")
######################################################
qfit3<-CQEst(tau=0.5,p=1,q=1,L=30,X=Y,B=1000,W=W)
phybrid3<-2*pt(-abs(qfit3$thetahat/qfit3$sdE),df=T-4)
write.csv(qfit3$thetahat,file="coefrobust-tau05.csv")
write.csv(phybrid3,file="pvaluerobust-tau05.csv")
#######################################################
qfit4<-CQEst(tau=0.75,p=1,q=1,L=30,X=Y,B=1000,W=W)
phybrid4<-2*pt(-abs(qfit4$thetahat/qfit4$sdE),df=T-4)
write.csv(qfit4$thetahat,file="coefrobust-tau075.csv")
write.csv(phybrid4,file="pvaluerobust-tau075.csv")
#######################################################
qfit5<-CQEst(tau=0.9 ,p=1,q=1,L=30,X=Y,B=1000,W=W)
phybrid5<-2*pt(-abs(qfit5$thetahat/qfit5$sdE),df=T-4)
write.csv(qfit5$thetahat,file="coefrobust-tau09.csv")
write.csv(phybrid5,file="pvaluerobust-tau09.csv")
#######################################################
coef01<-read.csv("coefrobust-tau01.csv",header = TRUE)
coef025<-read.csv("coefrobust-tau025.csv",header = TRUE)
coef05<-read.csv("coefrobust-tau05.csv",header = TRUE)
coef075<-read.csv("coefrobust-tau075.csv",header = TRUE)
coef09<-read.csv("coefrobust-tau09.csv",header = TRUE)
pvalue01<-read.csv("pvaluerobust-tau01.csv", header = TRUE)
pvalue025<-read.csv("pvaluerobust-tau025.csv", header = TRUE)
pvalue05<-read.csv("pvaluerobust-tau05.csv", header = TRUE)
pvalue075<-read.csv("pvaluerobust-tau075.csv", header = TRUE)
pvalue09<-read.csv("pvaluerobust-tau09.csv", header = TRUE)
...
ggplot(data, aes(x=Tau, y=Commodity, fill=alpha1))+
  ...
  scale_fill_gradientn(colors = c("red4","red","white","blue"), values = scales::rescale((c(-10,-2,0,2))), limits= c(-10,2), name = "")+
...
ggplot(data, aes(x=Tau, y=Commodity, fill=beta))+
  geom_tile(color="white")+
  scale_fill_gradientn(colors = c("red","white","blue"), values = scales::rescale((c(-1.5,0,2))), limits= c(-1.5,2), name = "")+
  theme_minimal(base_size = 12)+
  theme(axis.text.y=element_text(size = 7), axis.text.x=element_text(size = 10), panel.grid = element_blank())+
  labs(x="",y="")
...
ggplot(data, aes(x=Tau, y=Commodity, fill=lambda))+
  geom_tile(color="white")+
  scale_fill_gradientn(colors = c("red3","red","white","blue","blue4"), values = scales::rescale((c(-4,-2,0,2,10.5))), limits= c(-4,10.5), name = "")+
  theme_minimal(base_size = 12)+
  theme(axis.text.y=element_text(size = 7), axis.text.x=element_text(size = 10), panel.grid = element_blank())+
  labs(x="",y="")
########################################