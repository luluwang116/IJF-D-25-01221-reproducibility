  ...
  for (l in 1:M)
  {
    ...
    for (i in 1:N) {
      ...
      for (attempt in 1:10) {
        ...
        if (inherits(garch1_1, "try-error")) next
        
        theta <- coef(garch1_1)
        
        if (isTRUE(garch1_1@fit$convergence == 0) &&
            length(theta) == ncol(Theta_int) &&
            all(is.finite(theta))) {
          ok <- TRUE
          break
        }
      ...
      if (!ok) {
        stop(sprintf("GARCH fitting failed after 10 attempts: l=%d, i=%d", l, i))
      }
      
      Theta_int[i, ] <- as.vector(theta)#initial value for QMLE
      intc=rowMeans(X[,1:5]^2) #initial value of X_t^2 and h_t for t<=0 
      #initial value can also be mean(X^2) or X[1]^2 or Theta_int[1]/(1-Theta_int[2]-Theta_int[3]) 
      #the influence of intial value is small
    }
